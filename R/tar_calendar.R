...
build_schedule_for_page <- function(schedule_file) {
  ...
  grouped_data <- split(table_data, table_data$group, drop = TRUE)
  tibble::tibble(
    group = names(grouped_data),
    data = lapply(grouped_data, function(group_data) {
      dplyr::select(group_data, -group)
    })
  )
}
...
build_ical <- function(schedule_file, base_url, page_suffix, course_number) {
  ...
  events <- map_dfr(seq_len(nrow(schedule)), function(row_number) {
    row <- schedule[row_number, ]
    ic_event(
      start = as.POSIXct(row$date[[1]], format = "%Y-%m-%d"),
      end = as.POSIXct(row$event_end[[1]], format = "%Y-%m-%d") + 24 * 60 * 60,
      summary = row$summary[[1]],
      more_properties = TRUE,
      event_properties = c(
        "DESCRIPTION" = row$url[[1]],
        "DTSTAMP" = dtstamp
      )
    )
  }) |>
